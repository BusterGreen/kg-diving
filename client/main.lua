...
local function callCops()
    ...
    if call == chance then
        TriggerServerEvent('kg-diving:server:CallCops', coords)
    end
end
...
local function takeCoral(coral)
    ...
    KGCore.Functions.Progressbar("take_coral", Lang:t("info.collecting_coral"), times * 1000, false, true, {
        ...
        TriggerServerEvent('kg-diving:server:TakeCoral', currentDivingLocation.area, coral, true)
        ClearPedTasks(ped)
...
local function setDivingLocation(divingLocation)
    if currentDivingLocation.area ~= 0 then
        for k in pairs(Config.CoralLocations[currentDivingLocation.area].coords.Coral) do
            if Config.UseTarget then
                exports['kg-target']:RemoveZone(k)
            else
    ...
    for k, v in pairs(Config.CoralLocations[currentDivingLocation.area].coords.Coral) do
        if Config.UseTarget then
            exports['kg-target']:AddBoxZone('diving_coral_zone_'..k, v.coords, v.length, v.width, {
            ...
            zones[k]:onPlayerInOut(function(inside)
                if inside then
                    currentArea = k
                    exports['kg-core']:DrawText(Lang:t("info.collect_coral_dt"))
                else
                    currentArea = 0
...
local function sellCoral()
    ...
    KGCore.Functions.Progressbar("sell_coral_items", Lang:t("info.checking_pockets"), math.random(2000, 4000), false, true, {}, {}, {}, {}, function() -- Done
        ClearPedTasks(playerPed)
        TriggerServerEvent('kg-diving:server:SellCoral')
...
local function createSeller()
    for i = 1, #Config.SellLocations do
        ...
        if Config.UseTarget then
            exports['kg-target']:AddTargetEntity(ped, {
                options = {
                    {
            ...
            zone:onPlayerInOut(function(inside)
                if inside then
                    inSellerZone = true
                    exports['kg-core']:DrawText(Lang:t("info.sell_coral_dt"))
                else
                    inSellerZone = false
                    exports['kg-core']:HideText()
                end
...
RegisterNetEvent('KGCore:Client:OnPlayerLoaded', function()
    KGCore.Functions.TriggerCallback('kg-diving:server:GetDivingConfig', function(config, area)
...
RegisterNetEvent('kg-diving:client:NewLocations', function()
    KGCore.Functions.TriggerCallback('kg-diving:server:GetDivingConfig', function(config, area)
        Config.CoralLocations = config
        setDivingLocation(area)
    end)
...
RegisterNetEvent('kg-diving:client:UpdateCoral', function(area, coral, bool)
    Config.CoralLocations[area].coords.Coral[coral].PickedUp = bool
end)

RegisterNetEvent('kg-diving:client:CallCops', function(coords, msg)
...
RegisterNetEvent("kg-diving:client:setoxygenlevel", function()
    if oxgenlevell == 0 then
       ...
       KGCore.Functions.Notify(Lang:t("success.tube_filled"), 'success')
       TriggerServerEvent('kg-diving:server:removeItemAfterFill')
    else
        KGCore.Functions.Notify(Lang:t("error.oxygenlevel", {oxygenlevel = oxgenlevell}), 'error')
    end
end)
    ...
    if iswearingsuit == false then
        if oxgenlevell > 0 then
            ...
            if not IsPedSwimming(ped) and not IsPedInAnyVehicle(ped) then
                gearAnim()
                KGCore.Functions.Progressbar("equip_gear", Lang:t("info.put_suit"), 5000, false, true, {}, {}, {}, {},
                    function() -- Done
                        deleteGear()
                ...
                KGCore.Functions.Notify(Lang:t("error.not_standing_up"), 'error')
            end
        else
            KGCore.Functions.Notify(Lang:t("error.need_otube"), 'error')
        end
    elseif iswearingsuit == true then
        gearAnim()
        KGCore.Functions.Progressbar("remove_gear", Lang:t("info.pullout_suit"), 5000, false, true, {}, {}, {}, {},
            function() -- Done
                SetEnableScuba(ped, false)
                ...
                ClearPedTasks(ped)
                deleteGear()
                KGCore.Functions.Notify(Lang:t("success.took_out"))
                TriggerServerEvent("InteractSound_SV:PlayOnSource", nil, 0.25)
                iswearingsuit = false
                oxgenlevell = oxgenlevell
            end)
